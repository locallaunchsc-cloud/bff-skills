 ...
 *
 * Uses Commander.js for CLI argument parsing per BFF convention.
 * Cooldown persistence is file-based to survive process restarts.
 */
...
// Cooldown file path (persists across invocations)
const COOLDOWN_FILE = join(dirname(new URL(import.meta.url).pathname), ".cooldown.json");
...
function skillError(code: string, message: string, next: string): void {
...
function getWalletAddress(): string {
  ...
  if (!addr) {
    skillError("no_wallet", "No wallet address found. Set STACKS_ADDRESS env var.", "Configure wallet");
    process.exit(1);
  }
  return addr;
}

// -- File-based cooldown persistence
...
function saveCooldown(key: string, timestamp: number): void {
  const cooldowns = loadCooldowns();
  cooldowns[key] = timestamp;
  try {
    const dir = dirname(COOLDOWN_FILE);
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
    writeFileSync(COOLDOWN_FILE, JSON.stringify(cooldowns, null, 2));
...
async function doctor(): Promise<void> {
  ...
  checks["cooldown_persistence"] = { ok: true, detail: `File-based at ${COOLDOWN_FILE}` };
  const allOk = Object.values(checks).every(c => c.ok);
  const blockers = Object.entries(checks).filter(([, c]) => !c.ok).map(([k, c]) => `${k}: ${c.detail}`);
  output({
    status: allOk ? "success" : "blocked",
    action: allOk ? "Environment ready. Run: run --action=assess to check position drift." : "Fix blockers before proceeding",
...
async function runPlan(poolId: string, address: string, binWidth: number, force: boolean): Promise<void> {
  ...
  const profitable = dailyFeeEstimate * 1 > gasCostStx * 300;
  output({
    status: "success",
    action: profitable
...
async function runExecute(
  ...
  // File-based cooldown check (persists across process restarts)
  const cooldownKey = `${poolId}:${address}`;
  const lastRun = getCooldown(cooldownKey);
  ...
  // Spending limit checks - sBTC (token X)
  ...
  // FIX: Spending limit check - STX (token Y) - validates against totalWithdrawY
  const withdrawYinStx = plan.totalWithdrawY / 1_000_000;
  if (withdrawYinStx > maxStx) {
    blocked("exceeds_stx_limit", `Withdraw ${withdrawYinStx.toFixed(2)} STX > max ${maxStx} STX`, "Reduce position or increase --max-stx");
    return;
  }
  // Persist cooldown to file
  saveCooldown(cooldownKey, Date.now());
  ...
  output({
    ...
    data: {
      ...
      safety: {
        ...
        cooldownPersistence: "file-based",
        forceOverride: force,
      },
      timestamp: new Date().toISOString(),
    },
...
program
  ...
  .description("Detects out-of-range HODLMM positions and rebalances bin placement");
...
program
  ...
  .action(async () => {
    output({
      ...
      data: {
        required: ["fetch (built-in)", "commander"],
...
program
  .command("run")
  .description("Execute assess, plan, or execute action")
  .requiredOption("--action <action>", "Action to run: assess | plan | execute")
  .requiredOption("--pool-id <poolId>", "HODLMM pool ID (e.g. dlmm_3)")
  .option("--address <address>", "Override wallet address")
  .option("--bin-width <width>", "Bin width for rebalance", String(DEFAULT_BIN_WIDTH))
  .option("--confirm", "Confirm execution", false)
  .option("--force", "Override crisis regime block", false)
  .option("--max-sbtc <sats>", "Max sBTC sats to withdraw", String(DEFAULT_MAX_SBTC_SATS))
  .option("--max-stx <stx>", "Max STX to withdraw", String(DEFAULT_MAX_STX))
  .action(async (opts) => {
    const address = opts.address || getWalletAddress();
    const binWidth = parseInt(opts.binWidth, 10);
    const maxSbtc = parseInt(opts.maxSbtc, 10);
    const maxStx = parseInt(opts.maxStx, 10);
    switch (opts.action) {
      case "assess":
        await runAssess(opts.poolId, address);
        break;
      case "plan":
        await runPlan(opts.poolId, address, binWidth, opts.force);
        break;
      case "execute":
        await runExecute(opts.poolId, address, binWidth, opts.confirm, opts.force, maxSbtc, maxStx);
        break;
      default:
        skillError("unknown_action", `Unknown action: ${opts.action}`, "Use --action=assess|plan|execute");
    }
  });
